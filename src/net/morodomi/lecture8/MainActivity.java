...
public class MainActivity extends Activity {
	...
	protected void onResume() {
		...
		if (token == null || tokenSecret == null) {
			...
			try {
				String url = Config.provider.retrieveRequestToken(
						Config.consumer, Config.TWITTER_CALLBACK);
	...
	protected void onNewIntent(Intent intent) {
		...
		if (uri != null && uri.toString().startsWith(Config.TWITTER_CALLBACK)) {
			...
			try {
				Log.d(Config.TAG, "verifier:" + verifier);
				Config.provider.retrieveRequestToken(Config.consumer, verifier);

				// save tokens to preferences.
				SharedPreferences pref = getSharedPreferences(Config.PREFERENCES_NAME,
						MODE_PRIVATE);
